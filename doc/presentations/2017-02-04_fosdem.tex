...
\begin{document}
 ...
 \begin{frame}
  ...
  \begin{itemize}
   ...
   \item{Python tutor at Teckids e.V.}
  \end{itemize}
 \end{frame}
 ...
 \begin{frame}
  ...
  \centering
  \includegraphics[width=.3\textwidth]{ex_schoolkid_shot4}
 \end{frame}

 \note[itemize]{
  \item{Explain that the game object is rendered depending on distance and heading difference.}
  \item{Explain the difficulties of creating a pseudo-3D view from flat images that we faced.}
 }
 ...
 \begin{frame}
  ...
  \begin{itemize}
   \item{Code size: >2 kloc (>3.5 kloc including comments)}
   \item{Commits: exactly 1000 \smiley!}
   \item{Age: First birthday was on 2017-01-19}
   \item{Active contributors: 3}
   \item{Tasks in issue tracker: 33 open, 48 closed}
  \end{itemize}
 \end{frame}

 \begin{frame}
  \frametitle{FOSDEM game}

  Go to \url{https://nightly.veripeditus.org} with any mobile browser.

  \begin{itemize}
   \item{Small testing game on the campus}
   \item{Game may be slow and may not work on all devices
    \begin{itemize}
     \item{Camera view only works on Firefox on Android}
     \item{Map view should work on most browsers}
    \end{itemize}
   }
   \item{Please ping us via e-mail or find us somewhere if you have bugs to look at}
   \item{Find the full code at: \url{https://github.com/Veripeditus/game-fosdem17}}
  \end{itemize}

  Start by finding the guy at the entrance of building K and listen carefully \smiley!
 \end{frame}
